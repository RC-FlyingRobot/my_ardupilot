...
function update()
    ...
    if not switch_on or not is_armed then
        if state ~= STATE_IDLE then
            ...
            gcs:send_text(6, "AutoFlight-A: STABILIZE restored (switch OFF)")
        end
        state            = STATE_IDLE
        hover_start_ms   = nil
        circle_start_ms  = nil
    ...
    elseif state == STATE_TAKEOFF then
        if rangefinder:has_data_orient(RF_ORIENT_DOWN) then
            ...
            if math.abs(current_alt - TAKEOFF_ALT_M) < ALT_TOLERANCE_M then
                ...
                gcs:send_text(6, string.format("AutoFlight-A: Reached %.2fm, hovering 3s", current_alt))
                state = STATE_HOVER
            end
        else
            ...
            if pos and -pos:z() >= TAKEOFF_ALT_M * 0.9 then
                hover_start_ms = millis()
                gcs:send_text(6, "AutoFlight-A: Hovering 3s (AHRS fallback)")
                state = STATE_HOVER
            end
        end
    ...
    elseif state == STATE_CIRCLE then
        if millis() - circle_start_ms >= circle_duration_ms then
            if vehicle:set_mode(LAND_MODE) then
                ...
                gcs:send_text(4, string.format("AutoFlight-A: Cannot enter LAND mode (%d)", land_fail_count))
                if land_fail_count >= 5 then
                    vehicle:set_mode(STABILIZE_MODE)
                    gcs:send_text(4, "AutoFlight-A: LAND failed 5x, STABILIZE fallback")
                    land_fail_count = 0
                    state = STATE_IDLE
                end
            end
